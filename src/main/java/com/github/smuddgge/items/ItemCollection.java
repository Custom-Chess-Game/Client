package com.github.smuddgge.items;
...
public class ItemCollection {
    ...
    public void calculate() {
        ...
        for (Item item : this.items) {
            ...
            int columnMultiplier = (Client.getConfig().getScreenSize().get(0) + this.padding) / (this.columns + 1);
            int rowMultiplier = (Client.getConfig().getScreenSize().get(1) + this.padding) / (amountPerColumn + 1);

            // Difference from default screen size
            double sizeMultiplierX = Client.getConfig().getScreenSize().get(0) / (double)1000;
            double sizeMultiplierY = Client.getConfig().getScreenSize().get(1) / (double)600;
